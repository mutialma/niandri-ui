            ...
            {myTickets.map(t => {
              ...
              return (
                <tr key={t.id}>
                  ...
                  <td>
                    {t.status !== 'closed' && (
                      <button style={{ padding: '4px 10px', background: C.greenLight, border: `1px solid ${C.green}33`, borderRadius: 6, cursor: 'pointer', fontSize: '.72rem', color: C.green, fontWeight: 700, fontFamily: 'var(--font)' }}>✅ Selesai</button>
                    )}
                  </td>
                </tr>
              )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ background: '#fff', border: `1.5px solid ${C.border}`, borderRadius: 14, padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,.05)' }}>
            <div style={{ fontWeight: 800, fontSize: '.88rem', color: C.text, marginBottom: '0.75rem' }}>💳 Metode Terima Bayar</div>
            {[
              { icon: '💵', label: 'Cash',          desc: 'Terima tunai lalu setor' },
              { icon: '📱', label: 'Transfer Bank',  desc: 'BCA / BRI ke rekening Niandri' },
            ].map(m => (
              <div key={m.label} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '8px 0', borderBottom: `1px solid ${C.border}` }}>
                <span style={{ fontSize: '1.2rem' }}>{m.icon}</span>
                <div>
                  <div style={{ fontSize: '.82rem', fontWeight: 700 }}>{m.label}</div>
                  <div style={{ fontSize: '.72rem', color: C.muted }}>{m.desc}</div>
                </div>
              </div>
            ))}
          </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1rem' }}>
        ...
        <div style={{ background: invoices[0].status === 'belum' ? C.red : C.blue, borderRadius: 14, padding: '1.25rem', boxShadow: `0 4px 16px ${invoices[0].status === 'belum' ? C.red : C.blue}35` }}>
          ...
          {invoices[0].status === 'belum' && (
            <button
              onClick={() => setShowBayar(true)}
              style={{ marginTop: 10, background: 'rgba(255,255,255,.25)', border: '1px solid rgba(255,255,255,.4)', borderRadius: 8, padding: '6px 14px', color: '#fff', fontSize: '.75rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)' }}
            >
              💳 Bayar Sekarang
            </button>
          )}
        ...
        <div style={{ background: '#fff', border: `1.5px solid ${C.border}`, borderRadius: 14, padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,.05)' }}>
          ...
          {[
            { label: 'Paket',          value: 'Home 20Mbps'    },
            { label: 'Harga',          value: 'Rp 180.000/bln' },
            { label: 'Masa Aktif',     value: '01–31 Mei 2026' },
            { label: 'IP Address',     value: '10.8.1.45'      },
          ].map(s => (
      ...
      <div style={{ background: '#fff', border: `1.5px solid ${C.border}`, borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,.05)' }}>
        <div style={{ background: 'linear-gradient(90deg,#F97316,#EA580C)', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: '1rem' }}>🧾</span>
          <span style={{ fontWeight: 800, color: '#fff', fontSize: '.95rem' }}>Riwayat Tagihan</span>
        </div>
        <table className="data-table">
          ...
          <tbody>
            {invoices.map((inv, i) => (
              <tr key={i}>
                ...
                <td>
                  {inv.status === 'lunas'
                    ? <button style={{ padding: '4px 10px', background: C.blueLight, border: `1px solid ${C.blue}33`, borderRadius: 6, cursor: 'pointer', fontSize: '.72rem', color: C.blue, fontWeight: 700, fontFamily: 'var(--font)' }}>🖨 Cetak</button>
                    : <button onClick={() => setShowBayar(true)} style={{ padding: '4px 10px', background: C.orange, border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: '.72rem', color: '#fff', fontWeight: 700, fontFamily: 'var(--font)' }}>💳 Bayar</button>
                  }
                </td>
              </tr>
            ))}
      ...
      <div style={{ background: '#fff', border: `1.5px solid ${C.border}`, borderRadius: 14, padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,.05)' }}>
        <div style={{ fontWeight: 800, fontSize: '.9rem', color: C.text, marginBottom: '1rem' }}>🎫 Riwayat Tiket Gangguan</div>
        {[
          { id: 'TKT-038', masalah: 'PPPoE Offline', tgl: '12 Apr 2026', selesai: '12 Apr 2026 16:30', status: 'closed' },
          { id: 'TKT-021', masalah: 'Signal Lemah',  tgl: '03 Mar 2026', selesai: '04 Mar 2026 10:00', status: 'closed' },
        ].map((t, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i === 0 ? `1px solid ${C.border}` : 'none' }}>
            <div>
              <div style={{ fontSize: '.82rem', fontWeight: 700 }}>{t.masalah} <span style={{ fontFamily: 'var(--mono)', fontSize: '.72rem', color: C.orange }}>#{t.id}</span></div>
              <div style={{ fontSize: '.72rem', color: C.muted, marginTop: 2 }}>Dibuat: {t.tgl} · Selesai: {t.selesai}</div>
            </div>
            <span style={{ fontSize: '.7rem', fontWeight: 700, padding: '2px 9px', borderRadius: 100, background: C.greenLight, color: C.green }}>✓ Selesai</span>
          </div>
        ))}
      </div>
              ...
              {[
                ...
                <div key={m.id} className="pay-method-card">
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '.75rem', color: C.muted, marginTop: 2 }}>{m.desc}</div>
                  </div>
                </div>
              ))}
            </div>
